...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  const getKey = (menuItemId: number, variantName?: string) => `${menuItemId}:${variantName || ''}`;

  const addItem = (item: Omit<CartItem, 'quantity'>) => {
    setItems((prev) => {
      const key = getKey(item.menuItemId, item.variantName);
      const existing = prev.find((i) => getKey(i.menuItemId, i.variantName) === key);
      if (existing) {
        return prev.map((i) =>
          getKey(i.menuItemId, i.variantName) === key ? { ...i, quantity: i.quantity + 1 } : i
        );
      }
  ...
  const removeItem = (menuItemId: number, variantName?: string) => {
    ...
    setItems((prev) => prev.filter((i) => getKey(i.menuItemId, i.variantName) !== key));
  };
  ...
  const updateQuantity = (menuItemId: number, quantity: number, variantName?: string) => {
    ...
    const key = getKey(menuItemId, variantName);
    setItems((prev) =>
      prev.map((i) => (getKey(i.menuItemId, i.variantName) === key ? { ...i, quantity } : i))
    );
  };
