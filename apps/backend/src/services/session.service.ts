...
class SessionService {
  ...
  async getTableInfo(
    table: { id: number; capacity: number; tableNumber: number },
    deviceId?: string,
  ): Promise<TableInfoDTO> {
    const activeSessions = await sessionRepository.findAllActiveByTable(
      table.id,
    );
    ...
    if (deviceId) {
      ...
      if (hostSession) {
        ...
        const participantRows =
          await participantRepository.findActiveByDeviceInSessions(
            deviceId,
  ...
  async createSession(restaurantId: number, dto: CreateSessionDTO) {
    ...
    if (participantRows.length > 0) {
      const matchedSession = activeSessions.find(
        (s) => s.id === participantRows[0]!.sessionId,
      )!;
      const participants = await participantRepository.findBySession(
        matchedSession.id,
      );
    ...
    if (availableSeats <= 0) {
      throw new AppError(
        400,
        "SEATS_NOT_AVAILABLE: Table is full. No seats available.",
      );
    }
