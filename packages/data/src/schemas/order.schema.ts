...
export const orderItems = pgTable("order_items", {
  ...
  // Per-item acceptance tracking
  acceptedByKitchen: text("accepted_by_kitchen").references(() => user.id, {
    onDelete: "set null",
  }),
  acceptedByWaiter: text("accepted_by_waiter").references(() => user.id, {
    onDelete: "set null",
  }),
  acceptedByKitchenAt: timestamp("accepted_by_kitchen_at"),
  acceptedByWaiterAt: timestamp("accepted_by_waiter_at"),
...
export const orderItemsRelations = relations(orderItems, ({ one }) => ({
  ...
  kitchenAcceptor: one(user, {
    fields: [orderItems.acceptedByKitchen],
    references: [user.id],
    relationName: "itemKitchenAcceptor",
  }),
  waiterAcceptor: one(user, {
    fields: [orderItems.acceptedByWaiter],
    references: [user.id],
    relationName: "itemWaiterAcceptor",
  }),
}));
