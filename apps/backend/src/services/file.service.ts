...
import { restaurants, menuItems, files, user } from "@menugo/data/schemas";
...
class FileService {
  async uploadFile(dto: UploadFileDTO) {
    ...
    if (file.size > config.storage.maxFileSize) {
      throw new AppError(
        400,
        `File size exceeds maximum of ${config.storage.maxFileSize / (1024 * 1024)}MB`,
      );
    }
    ...
    const key = await this.buildStorageKey(
      entityType,
      entityId,
      purpose,
      timestamp,
    ...
    const existingFile = await fileRepository.findByEntityAndPurpose(
      entityType,
      entityId,
      purpose,
    );
  ...
  async streamFile(fileId: number) {
    ...
    const storage = getStorageProvider();
    const { stream, contentType, contentLength } = await storage.getStream(
      file.key,
    );
  ...
  private async updateEntityImageField(
    ...
    try {
      if (entityType === "restaurant") {
        ...
        await db.update(user).set({ image: url }).where(eq(user.id, entityId));
      }
    } catch {
