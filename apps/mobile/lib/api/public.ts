...
class PublicAPI {
  ...
  async joinSession(joinCode: string, deviceId: string, participantName?: string) {
    return this.request<{ success: boolean; data: any }>('/api/public/session/join', {
      method: 'POST',
      body: JSON.stringify({ joinCode, deviceId, participantName }),
    });
  }

  async getSessionStatus(sessionId: number) {
    return this.request<{ success: boolean; data: any }>(`/api/public/session/${sessionId}`);
  }

  async placeOrder(sessionId: number, deviceId: string, items: any[], notes?: string) {
    return this.request<{ success: boolean; data: any }>(`/api/public/session/${sessionId}/order`, {
      method: 'POST',
      body: JSON.stringify({ deviceId, items, notes }),
    });
  }
