...
import { kitchenMembers, kitchenMenuItems, kitchens, menuItems } from "@menugo/data/schemas";

class KitchenRepository {
  async findByRestaurant(restaurantId: number) {
    return db.select().from(kitchens).where(eq(kitchens.restaurantId, restaurantId));
  }
  ...
  async create(restaurantId: number, name: string) {
    const [k] = await db.insert(kitchens).values({ restaurantId, name }).returning();
    return k;
  }

  async update(id: number, data: { name?: string; isActive?: boolean }) {
    const [k] = await db.update(kitchens).set({ ...data, updatedAt: new Date() }).where(eq(kitchens.id, id)).returning();
    return k;
  }

  async delete(id: number) {
    const [k] = await db.delete(kitchens).where(eq(kitchens.id, id)).returning();
    return k;
  }

  async setMenuItemKitchen(menuItemId: number, kitchenId: number | null) {
    const result = await db.transaction(async (tx) => {
      await tx.delete(kitchenMenuItems).where(eq(kitchenMenuItems.menuItemId, menuItemId));
      if (!kitchenId) return null;
      const [m] = await tx.insert(kitchenMenuItems).values({ kitchenId, menuItemId }).returning();
      return m;
    });
    return result;
  }
  ...
  async addMember(kitchenId: number, userId: string) {
    const [m] = await db.insert(kitchenMembers).values({ kitchenId, userId }).returning();
    return m;
  }
  ...
  async findMembersByKitchenId(kitchenId: number) {
    return db.select({ userId: kitchenMembers.userId }).from(kitchenMembers).where(eq(kitchenMembers.kitchenId, kitchenId));
  }
  ...
  async findMemberUserIdsByKitchenIds(kitchenIds: number[]) {
    ...
    const rows = await db
      ...
      .where(and(inArray(kitchenMembers.kitchenId, kitchenIds), eq(kitchens.isActive, true)));
